# Generated by Django 5.0.7 on 2025-01-04 02:28

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ("Equipe", "0001_initial"),
    ]

    operations = [
        migrations.CreateModel(
            name="Servico",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                (
                    "Tipo_investimento",
                    models.CharField(
                        blank=True,
                        choices=[("Capex", "Capex"), ("Opex", "Opex"), ("0", "0")],
                        default="",
                        max_length=50,
                        null=True,
                    ),
                ),
                ("Numero_Servico", models.CharField(max_length=100, unique=True)),
                ("PEP", models.CharField(max_length=40, unique=True)),
                (
                    "Servico",
                    models.CharField(
                        choices=[
                            ("Plano de Manutenção", "Plano de Manutenção"),
                            ("Desenho tecnico", "Desenho tecnico"),
                            ("Ocorrência", "Ocorrência"),
                        ],
                        default="",
                        max_length=50,
                    ),
                ),
                (
                    "Status_SAP",
                    models.CharField(
                        blank=True,
                        choices=[
                            ("ABER/ABER", "ABER/ABER"),
                            ("LIB/LOG", "LIB/LOG"),
                            ("LIB/ATEC", "LIB/ATEC"),
                            ("LIB/ENER", "LIB/ENER"),
                            ("ENCE/ENCE", "ENCE/ENCE"),
                            ("LIB/CKCP", "LIB/CKCP"),
                            ("LIB/ENTE", "LIB/ENTE"),
                            ("LIB/MED", "LIB/MED"),
                            ("LIB/COMS", "LIB/COMS"),
                            ("LIB/PEND", "LIB/PEND"),
                            ("LIB/CONC", "LIB/CONC"),
                            ("LIB/AVAL", "LIB/AVAL"),
                            ("LIB/ANTE", "LIB/ANTE"),
                            ("LIB/REC", "LIB/REC"),
                            ("LIB/DLT", "LIB/DLT"),
                            ("LIB/DEV", "LIB/DEV"),
                            ("LIB/DFEC", "LIB/DFEC"),
                            ("0", "0"),
                        ],
                        default="",
                        max_length=50,
                        null=True,
                    ),
                ),
                (
                    "Mês_servico",
                    models.CharField(
                        choices=[
                            ("Jan", "Jan"),
                            ("Fev", "Fev"),
                            ("Mar", "Mar"),
                            ("Abr", "Abr"),
                            ("Mai", "Mai"),
                            ("Jun", "Jun"),
                            ("Jul", "Jul"),
                            ("Ago", "Ago"),
                            ("Set", "Set"),
                            ("Out", "Out"),
                            ("Nov", "Nov"),
                            ("Dez", "Dez"),
                        ],
                        default="",
                        max_length=4,
                    ),
                ),
                ("Ano_servico", models.CharField(default="", max_length=4)),
                ("data_da_solicitacao", models.DateField(default="")),
                (
                    "tipo_servico",
                    models.CharField(
                        choices=[
                            ("SUBS DE POSTE", "SUBS DE POSTE"),
                            ("SUBS DE TRAFO", "SUBS DE TRAFO"),
                            ("SUBS CONDUTOR", "SUBS CONDUTOR"),
                            ("APOIO", "APOIO"),
                            ("SUBS DE CHAVE FACA", "SUBS DE CHAVE FACA"),
                            ("SUBS DE CHAVE FUS", "SUBS DE CHAVE FUS"),
                            ("SUBS CONDUTOR", "SUBS CONDUTOR"),
                            ("SUBS DE POSTE E TRAFO", "SUBS DE POSTE E TRAFO"),
                            ("SUBS DE ESTRUTURA MT", "SUBS DE ESTRUTURA MT"),
                            ("CONEXÃO", "CONEXÃO"),
                            ("PODA", "PODA"),
                            ("CAVA", "CAVA"),
                            ("ESPAÇADOR", "ESPAÇADOR"),
                            ("TALA", "TALA"),
                            ("ELO FUSIVEL", "ELO FUSIVEL"),
                            ("RELIGADOR", "RELIGADOR"),
                            ("REGULADOR", "REGULADOR"),
                            ("MUFLA", "MUFLA"),
                            ("RECONDUTORAMENTO DE BT", "RECONDUTORAMENTO DE BT"),
                            ("RECONDUTORAMENTO DE MT", "RECONDUTORAMENTO DE MT"),
                            ("PODA GRANDE POSTE", "PODA GRANDE POSTE"),
                            ("ABATE", "ABATE"),
                            ("ESPAÇADOR", "ESPAÇADOR"),
                            ("REGULAGEM", "REGULAGEM"),
                            ("GLV-CONEXÃO-ARMAÇÃO", "GLV-CONEXÃO-ARMAÇÃO"),
                            ("SUBS DE ESTRUTURA BT", "SUBS DE ESTRUTURA BT"),
                            ("FLY-TAP", "FLY-TAP"),
                            ("ARMAÇÃO E OUTROS", "ARMAÇÃO E OUTROS"),
                            ("APRUMO DE POSTE", "APRUMO DE POSTE"),
                            ("FECHAMENTO DE CAVA", "FECHAMENTO DE CAVA"),
                            ("ELO FUSIVEL", "ELO FUSIVEL"),
                            ("ABATE", "ABATE"),
                            ("IMPLANTAÇÃO DE POSTE", "IMPLANTAÇÃO DE POSTE"),
                            ("IMPLANTAÇÃO DE TRAFO", "IMPLANTAÇÃO DE TRAFO"),
                            ("PODA PEQUENO POSTE", "PODA PEQUENO POSTE"),
                            ("PODA MENOR DE 5 GALHOS", "PODA MENOR DE 5 GALHOS"),
                            ("RETIRAR EQP MEDIÇÃO", "RETIRAR EQP MEDIÇÃO"),
                        ],
                        default="",
                        max_length=50,
                    ),
                ),
                (
                    "Status",
                    models.CharField(
                        choices=[
                            ("Programação", "Programação"),
                            ("Andamento", "Andamento"),
                            ("Espera", "Espera"),
                            ("Concluido", "Concluido"),
                            ("Fechamento", "Fechamento"),
                            ("Almoxerifado", "Almoxerifado"),
                            ("Pagamento", "Pagamento"),
                            ("Recebido", "Recebido"),
                        ],
                        default="",
                        max_length=25,
                    ),
                ),
                (
                    "Municipio",
                    models.CharField(
                        choices=[
                            ("ALVORADA", "ALVORADA"),
                            ("ARROIO DOS RATOS", "ARROIO DOS RATOS"),
                            ("BARRA DO RIBEIRO", "BARRA DO RIBEIRO"),
                            ("BUTIÁ", "BUTIÁ"),
                            ("CHARQUEADAS", "CHARQUEADAS"),
                            ("ELDORADO DO SUL", "ELDORADO DO SUL"),
                            ("GUAÍBA", "GUAÍBA"),
                            ("MARIANA PIMENTEL", "MARIANA PIMENTEL"),
                            ("MINAS DO LEÃO", "MINAS DO LEÃO"),
                            ("PANTANO GRANDE", "PANTANO GRANDE"),
                            ("PORTO ALEGRE", "PORTO ALEGRE"),
                            ("SÃO JERÔNIMO", "SÃO JERÔNIMO"),
                            ("VIAMÃO", "VIAMÃO"),
                            ("AMARAL FERRADOR", "AMARAL FERRADOR"),
                            ("ARAMBARÉ", "ARAMBARÉ"),
                            ("ARROIO DO PADRE", "ARROIO DO PADRE"),
                            ("ARROIO GRANDE", "ARROIO GRANDE"),
                            ("BAGÉ", "BAGÉ"),
                            ("BARAO DO TRIUNFO", "BARAO DO TRIUNFO"),
                            ("CAMAQUÃ", "CAMAQUÃ"),
                            ("CANDIOTA", "CANDIOTA"),
                            ("CANGUÇU", "CANGUÇU"),
                            ("CAPÃO DO LEÃO", "CAPÃO DO LEÃO"),
                            ("CERRITO", "CERRITO"),
                            ("CERRO GRANDE DO SUL", "CERRO GRANDE DO SUL"),
                            ("CHUÍ", "CHUÍ"),
                            ("CHUVISCA", "CHUVISCA"),
                            ("CRISTAL", "CRISTAL"),
                            ("DOM FELICIANO", "BDOM FELICIANO"),
                            ("DOM PEDRITO", "DOM PEDRITO"),
                            ("ENCRUZILHADA DO SUL", "ENCRUZILHADA DO SUL"),
                            ("HERVAL", "HERVAL"),
                            ("HULHA NEGRA", "HULHA NEGRA"),
                            ("JAGUARÃO", "JAGUARÃO"),
                            ("LAVRAS DO SUL", "LAVRAS DO SUL"),
                            ("MORRO REDONDO", "MORRO REDONDO"),
                            ("PEDRAS ALTAS", "PEDRAS ALTAS"),
                            ("PEDRO OSÓRIO", "PEDRO OSÓRIO"),
                            ("PELOTAS", "PELOTAS"),
                            ("JAGUARÃO", "JAGUARÃO"),
                            ("PINHEIRO MACHADO", "PINHEIRO MACHADO"),
                            ("PIRATINI", "PIRATINI"),
                            ("RIO GRANDE", "RIO GRANDE"),
                            ("SÃO LOURENÇO DO SUL", "SÃO LOURENÇO DO SUL"),
                            ("SENTINELA DO SUL", "SENTINELA DO SUL"),
                            ("Sertão Santana", "Sertão Santana"),
                            ("SANTA VITÓRIA DO PALMAR", "SANTA VITÓRIA DO PALMAR"),
                            ("TAPES", "TAPES"),
                        ],
                        default="",
                        max_length=50,
                    ),
                ),
                ("Endereco", models.CharField(max_length=100)),
                (
                    "data_programacao",
                    models.DateField(blank=True, default="", null=True),
                ),
                ("tecnico", models.CharField(default=False, max_length=80)),
                (
                    "evidencias",
                    models.BooleanField(default=False, verbose_name="Evidências"),
                ),
                ("AES_ACOS", models.BooleanField(default=False)),
                ("As_built", models.BooleanField(default=False)),
                ("Medicao", models.BooleanField(default=False)),
                ("andamento", models.IntegerField(default=0, editable=False)),
                ("Requisicao_ODI", models.BooleanField(default=False)),
                ("Requisicao_ODD", models.BooleanField(default=False)),
                (
                    "Valor_parcial",
                    models.DecimalField(
                        blank=True, decimal_places=2, max_digits=10, null=True
                    ),
                ),
                (
                    "Valor_final",
                    models.DecimalField(
                        blank=True, decimal_places=2, max_digits=10, null=True
                    ),
                ),
                (
                    "Valor_pago",
                    models.DecimalField(
                        blank=True, decimal_places=2, max_digits=10, null=True
                    ),
                ),
                (
                    "desenho_servico",
                    models.FileField(
                        blank=True,
                        null=True,
                        upload_to="media/desenhoservico/arquivos/",
                    ),
                ),
                (
                    "foto_antes",
                    models.FileField(
                        blank=True,
                        null=True,
                        upload_to="media/desenhoservico/arquivos/",
                    ),
                ),
                (
                    "foto_depois",
                    models.FileField(
                        blank=True,
                        null=True,
                        upload_to="media/desenhoservico/arquivos/",
                    ),
                ),
                ("Observacao", models.TextField(blank=True, max_length=800, null=True)),
                (
                    "Equipe",
                    models.ForeignKey(
                        blank=True,
                        default="",
                        max_length=50,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        to="Equipe.equipe",
                    ),
                ),
            ],
            options={
                "permissions": [("acesso_servicos", "Acesso ao gestao de servicos")],
            },
        ),
    ]
